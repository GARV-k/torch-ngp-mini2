...
if __name__ == '__main__':
    ...
    paths = get_final_objs(cat_dict, 2,1)
    print()
    print(paths)
    print()
    ...
    if opt.ff:
        ...
        from sdf.network_tcnn import SDFNetwork,SDFNetwork_GE      
    else:
    ...
    models = []
    for idx in range(n):
        model = SDFNetwork_GE( tcnn_network, encoding="hashgrid")
        models.append(model)
    ...
    if opt.test:
        ...
        valid_loaders = []
        for i in range(n):
            train_dataset = SDFDataset(paths[i], size=200, num_samples=2**18)
    ...
    #         trainer.train(subset_train_loaders, subset_valid_loaders,5)
        trainer = Trainer('ngp', n, models, w,workspace=opt.workspace1, enc_optimizer=enc_optimizer,  net_optimizer=net_optimizer, criterion=criterion, ema_decay=0.95, fp16=opt.fp16, lr_scheduler=scheduler, use_checkpoint='latest', eval_interval=1)
        trainer.train(train_loaders, valid_loaders,20,subset_n)
        for idx, model in enumerate(models):
            enc_state_dict = model.encoder.state_dict()
            path = f"saved_models/with_GE/Low_size/enc_{idx}.pth"
            torch.save(enc_state_dict, path)
            print("Completed saving path of enc-models")
        # mlp_state_dict = model.backbone.state_dict()
        # path = f"saved_models/mlp.pth"
        # torch.save(mlp_state_dict, path)

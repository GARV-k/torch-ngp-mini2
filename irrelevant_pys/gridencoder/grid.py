...
class _grid_w_encode(Function):
    @staticmethod
    @custom_fwd
    def forward(ctx, inputs, embeddings, offsets, w, per_level_scale, base_resolution, calc_grad_inputs=False, gridtype=0, align_corners=False, interpolation=0):
        # inputs: [B, D], float in [0, 1]
        # embeddings: [sO, C], float
        # offsets: [L + 1], int
        # RETURN: [B, F], float
        ...
        if calc_grad_inputs:
            ...
            dy_dx = None
        # new_embed = w.embeddings@embeddings
        new_embed = w@embeddings
        _backend.grid_encode_forward(inputs,new_embed, offsets, outputs, B, D, C, L, S, H, dy_dx, gridtype, align_corners, interpolation)
    ...
    def backward(ctx, grad):
        ...
        H_H = embeddings.shape[0]
        # Initialize gradient tensor for w with shape [H, H]
        grad_w = torch.zeros((H_H, H_H), device=embeddings.device, dtype=embeddings.dtype)
        ...
        # Backpropagate through the grid_encode function
        _backend.grid_encode_backward(grad, inputs, embeddings, offsets, grad_embeddings, B, D, C, L, S, H, dy_dx, grad_inputs, gridtype, align_corners, interpolation)

        # Calculate gradient with respect to w
        # Here, we are reducing `grad[i]` along the batch dimension before multiplication
        # `grad` has shape [L, B, C] which needs to be reduced to match the shape of w
        # Sum over the batch dimension `B` before performing `einsum`
        # for i in range(L):
        #     # Summing over batch dimension
        #     summed_grad = grad[i].sum(dim=1)  # [C]
            
        #     # `summed_grad` is now [C]
        #     # `embeddings` is [H_H, C]
        #     grad_w += torch.einsum('i,jk->ik', summed_grad, embeddings)
        grad_w+=grad_embeddings@embeddings.T

        # Convert gradients for inputs back to original dtype
        if dy_dx is not None:
            grad_inputs = grad_inputs.to(inputs.dtype)

        # Return gradients in the same order as the inputs to the forward method
        return grad_inputs, grad_embeddings, None, grad_w, None, None, None, None, None, None

        
grid_encode = _grid_encode.apply
grid_w_encode = _grid_w_encode.apply
...
class GridEncoder(nn.Module):
    ...
    def w_forward(self, inputs, bound=1,w=None):
        # inputs: [..., input_dim], normalized real world positions in [-bound, bound]
        # return: [..., num_levels * level_dim]
        ...
        outputs = grid_w_encode(inputs, self.embeddings, self.offsets, w,self.per_level_scale, self.base_resolution, inputs.requires_grad, self.gridtype_id, self.align_corners, self.interp_id)
        outputs = outputs.view(prefix_shape + [self.output_dim])
    ...
    def w_grad_total_variation(self, weight=1e-7, inputs=None, bound=1, B=1000000, w=None):
        # inputs: [..., input_dim], float in [-b, b], location to calculate TV loss.
        ...
        # If w is provided, use it to transform the embeddings
        if w is not None:
            transformed_embeddings = w @ self.embeddings
        else:
            transformed_embeddings = self.embeddings
        ...
        # Use transformed embeddings in the TV loss calculation
        _backend.grad_total_variation(inputs, transformed_embeddings, self.embeddings.grad, self.offsets, weight, B, D, C, L, S, H, self.gridtype_id, self.align_corners)
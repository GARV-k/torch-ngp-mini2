...
class Trainer(object):
    ...
    def train(self, train_loaders, max_epochs,macro_batch_size):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                if epoch%2==0:
                    for idx,model in enumerate(self.models):
                            enc_path = self.workspace+f'/GT_enc_{idx}.pth'
                            mlp_path = self.workspace+f'/GT_mlp.pth'
                            torch.save(model.encoder,enc_path)
                            print(f"the Encoder of model {idx+1} is saved at : {enc_path}")
                            torch.save(model.backbone,mlp_path)
                            print(f"the MLP of model {idx+1} is saved at : {mlp_path}")
    ...
    def w_train(self, mode,multiple_rots,train_loaders, max_epochs,macro_batch_size):
        if self.use_tensorboardX and self.local_rank == 0:
            self.writer = tensorboardX.SummaryWriter(os.path.join(self.workspace, "run", self.name))
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
                ...
                if epoch%1==0:
                    if mode == 'lora':
                        ...
                        w_path = self.workspace+'W.pth'
                        torch.save(self.W,w_path)
                        print(f"the W transform is saved at : {w_path}")
                    print("Completed saving path of models")
                torch.cuda.empty_cache()
